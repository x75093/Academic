...
if __name__ == '__main__':
    ...
    program = cl.Program(context, open('double_filter_2.cl').read()).build(options='')
    ...
    #Execute Derivative of Gaussian Function
    event_dertivative = program.gaussian_second_axis(queue, global_size, local_size, 
                        zero_derivative_out, 
                        first_derivative_out, 
    ...
    event_filter = program.gaussian_filter(queue, global_size_2, local_size_2, gpu_image_Wxx_filter_in, 
        gpu_image_Wyy_filter_in, gpu_image_Wxy_filter_in, 
        gpu_image_filter_out,
        local_memory_filter_Wxx, local_memory_filter_Wyy, local_memory_filter_Wxy,
        halo_2, width, height, buf_width_2, buf_height_2, filter_kernel)
    ...
    print 'compare_cl y first', host_image_Wyy[pt_x , pt]
    cor_x1 = filters.correlate1d(host_image64,filter_kernel_x64,0, mode = 'nearest')
    print 'compare scipy y first', cor_x1[pt_x , pt]

    # a = host_image[pt_x -4:pt_x +5, pt]
    # y = np.dot(filter_kernel_x, a)#.astype(np.float32)
    # print 'compare hand y first', y

    print 'compare_cl x', host_image_Wxx[pt_x , pt:pt+4]
    imx = np.zeros(host_image64.shape)
    filters.gaussian_filter(host_image64, (sigma,sigma), (0,1), imx, mode = 'nearest')
    print 'scipy gaussian x', imx[pt_x , pt:pt+4]

    cor_x2 = filters.correlate1d(cor_x1,filter_kernel_zero,1, mode = 'nearest')
    print 'scipy 1d correlated x', cor_x2[pt_x , pt:pt+4] 

    imy = np.zeros(host_image64.shape)
    filters.gaussian_filter(host_image64, (sigma,sigma), (1,0), imy)
    print 'scipy gaussian y', imy[pt_x , pt:pt+4]


    #print 'compare_cl x zero', host_image_Wyy[0, pt:pt+4]
    #print 'gause zero', host_image_Wyy[0, pt]
    #print 'filter', filter_kernel_x 
    #print 'total', t
